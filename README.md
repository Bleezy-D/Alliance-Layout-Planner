...
- **Real Kingshot map overlay** (toggleable) — the verified fixed map skeleton: King's Castle + turrets, all 90 permanent structures (4 fortresses, 12 sanctuaries, outposts, frontier lodges) with their no-build zones, and the King's exclusion zone. Positions are calibrated against in-game coordinates. In-game placement rules are enforced: nothing on structures/King's zone, no alliance buildings, nodes or traps in the Ruins ring, HQs only in Plains/Badlands, banners blocked in structure forbidden zones (cities exempt at fortresses/sanctuaries). Coordinate readouts match the game's axis exactly. All 6,499 resource nodes and the lake/mountain terrain are included and orientation-calibrated against in-game landmark fingerprints (structures, plus a uniquely-shaped border lake). If a node or lake looks off on your server, report its in-game X:Y.
...
- **Tabbed sidebar** — Build (tools, alliances, palette, presets), Map (overlay, legend, settings), Help (full in-app user guide, also reachable via the ? button on the map)
- **Legend** — full symbol reference on the Map tab with show/hide checkboxes for every layer
- **Pick up & carry** — click a selected group to lift it: it follows the cursor with live green/red placement validity, rotates mid-carry with R, and only sets down on a clear spot; Esc cancels
- **Rotate** — rotate any selection in exact 90° steps (R / Shift+R), obstructed rotations refuse safely
